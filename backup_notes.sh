...
if ! grep -q $SCRIPT_LOCATION /tmp/my_cron
then	
...
# Do backup
/usr/local/bin/gtimeout 180 /usr/local/bin/rsync -vaE --progress --delete-after "${HOME}/Library/Group Containers/group.com.apple.notes" "${HOME}/Library/Containers/com.apple.Notes" ${BACKUP_DRIVE}/ 2>&1 | /usr/bin/tee -a /tmp/rsync_notes.log
